...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:1142-1143](../shor.html#L1142-L1143)）
...
（[shor.html:1369-1371](../shor.html#L1369-L1371)、`tick()` 内で
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:1019-1024](../shor.html#L1019-L1024)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:1372](../shor.html#L1372)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:1403-1407](../shor.html#L1403-L1407)）は
...
上限判定に引っかかって「まだ、流れ着いた一枚がありません」という誤解を招く表示になる）。
peek/confirmに分離し、どちらも「現像完了」という同じ瞬間に合わせたことで
このズレを解消している。

### リセットのタイミング: 深夜0時ではなく朝7時

`appDayStr()` （[shor.html:941-945](../shor.html#L941-L945)）が「アプリ内の
...
`renderHome()`の前日結果判定（[shor.html:1181](../shor.html#L1181)）・
`getResultForDate()`の集計期間（[shor.html:1078-1102](../shor.html#L1078-L1102)、
...
`canView() === false` のときの扱いは、画面によって異なる（[screens.md](screens.md)の
「主・副ボタン」「scr-view」節参照）。

- **ホーム画面**（`#btn-see`「浜辺を見に行く」）: 文言・`disabled`の出し分けは
  廃止し、常時表示・常時押下可能にした。クリックで無条件に`openView("home")`
  する（[shor.html:1998](../shor.html#L1998)）。`canView()`の判定は
  `openView()`側に移り、`false`なら`peek_drift`すら呼ばずscr-view画面内で
  `showViewEmpty()`（[shor.html:1338-1343](../shor.html#L1338-L1343)）が
  「明日の朝　潮が満ちるころに届きます」と案内する
  （[shor.html:1284-1287](../shor.html#L1284-L1287)。モーダルではなく
  画面遷移後の常設表示で、「ホームへ」ボタンで戻る）。以前あった
  「投稿済みかどうかで文言を変える」分岐はこれより前に廃止済み
- **投稿完了画面**「浜辺を見に行く」（`#btn-give2see`、
  [shor.html:1881-1887](../shor.html#L1881-L1887)。文言は他の「見に行く」
  系ボタンと統一するため「流れ着いた一通を見る」から変更したが、
  `canView()`による判定ロジック自体は変更なし）: クリック時に
  `canView()`を判定し、`false`なら`notice-modal`で
  「今日のボトルメールは以上です　朝7時に、次の一通が流れ着きます」と表示する

ホーム画面の「入り口の判定が無くなった」ことで、閲覧枠切れでも一旦
scr-viewへ遷移するようになった点に注意（実際に枠が減るのは前述の通り
現像完了時点のままで変わらない）。
