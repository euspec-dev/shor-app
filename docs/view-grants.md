...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:741-742](../shor.html#L741-L742)）
...
- `consumeView()` （[shor.html:759-762](../shor.html#L759-L762)）
...
（[shor.html:888-895](../shor.html#L888-L895)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:837](../shor.html#L837)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:646-651](../shor.html#L646-L651)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:889](../shor.html#L889)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:922-926](../shor.html#L922-L926)）は
...
`todayStr()` （[shor.html:602-605](../shor.html#L602-L605)）はブラウザの
