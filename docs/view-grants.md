...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:1128-1129](../shor.html#L1128-L1129)）
...
（[shor.html:1358-1359](../shor.html#L1358-L1359)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:1278](../shor.html#L1278)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:1008-1013](../shor.html#L1008-L1013)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:1359](../shor.html#L1359)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:1392-1396](../shor.html#L1392-L1396)）は
...
`appDayStr()` （[shor.html:926-929](../shor.html#L926-L929)）が「アプリ内の
...
（[shor.html:930-934](../shor.html#L930-L934)）は同じ区切りでの前日を返す。
...
`renderHome()`の前日結果判定（[shor.html:1170](../shor.html#L1170)）・
`getResultForDate()`の集計期間（[shor.html:1064-1088](../shor.html#L1064-L1088)、
...
- **ホーム画面**（`#btn-see`「浜辺を見に行く」）: 文言・`disabled`の出し分けは
  ...
  する（[shor.html:1992](../shor.html#L1992)）。`canView()`の判定は
  `openView()`側に移り、`false`なら`peek_drift`すら呼ばずscr-view画面内で
  `showViewEmpty()`（[shor.html:1327-1332](../shor.html#L1327-L1332)）が
  「明日の朝　潮が満ちるころに届きます」と案内する
  （[shor.html:1273-1276](../shor.html#L1273-L1276)。モーダルではなく
  画面遷移後の常設表示で、「ホームへ」ボタンで戻る）。以前あった
  「投稿済みかどうかで文言を変える」分岐はこれより前に廃止済み
- **投稿完了画面**「流れ着いた一通を見る」（`#btn-give2see`、
  [shor.html:1874-1880](../shor.html#L1874-L1880)）: こちらは変更なし。
