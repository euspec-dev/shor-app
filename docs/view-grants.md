...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:729-730](../shor.html#L729-L730)）
...
（[shor.html:876-883](../shor.html#L876-L883)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:825](../shor.html#L825)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:634-639](../shor.html#L634-L639)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:877](../shor.html#L877)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:910-914](../shor.html#L910-L914)）は
...
`todayStr()` （[shor.html:590-593](../shor.html#L590-L593)）はブラウザの
...
- ホーム画面「誰かの感性に触れる」（[shor.html:1244-1256](../shor.html#L1244-L1256)）
- 投稿完了画面「誰かの感性に触れる」（[shor.html:1163-1169](../shor.html#L1163-L1169)）
