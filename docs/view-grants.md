...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:782-783](../shor.html#L782-L783)）
...
（[shor.html:958-959](../shor.html#L958-L959)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:907](../shor.html#L907)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:667-672](../shor.html#L667-L672)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:959](../shor.html#L959)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:992-996](../shor.html#L992-L996)）は
...
`appDayStr()` （[shor.html:622-625](../shor.html#L622-L625)）が「アプリ内の
...
（[shor.html:626-630](../shor.html#L626-L630)）は同じ区切りでの前日を返す。
...
`renderHome()`の前日結果判定（[shor.html:822](../shor.html#L822)）・
`getResultForDate()`の集計期間（[shor.html:725-745](../shor.html#L725-L745)、
...
- ホーム画面「誰かの感性に触れる」（[shor.html:1320-1332](../shor.html#L1320-L1332)）
- 投稿完了画面「誰かの感性に触れる」（[shor.html:1239-1245](../shor.html#L1239-L1245)）
