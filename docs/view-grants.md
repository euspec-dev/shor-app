...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:732-733](../shor.html#L732-L733)）
...
（[shor.html:879-886](../shor.html#L879-L886)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:828](../shor.html#L828)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:637-642](../shor.html#L637-L642)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:880](../shor.html#L880)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:913-917](../shor.html#L913-L917)）は
...
`todayStr()` （[shor.html:593-596](../shor.html#L593-L596)）はブラウザの
...
- ホーム画面「誰かの感性に触れる」（[shor.html:1247-1259](../shor.html#L1247-L1259)）
- 投稿完了画面「誰かの感性に触れる」（[shor.html:1166-1172](../shor.html#L1166-L1172)）
