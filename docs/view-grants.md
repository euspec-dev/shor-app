...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:1127-1128](../shor.html#L1127-L1128)）
...
- `consumeView()` （[shor.html:1145-1148](../shor.html#L1145-L1148)）
...
（[shor.html:1356-1357](../shor.html#L1356-L1357)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:1276](../shor.html#L1276)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:1007-1012](../shor.html#L1007-L1012)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:1357](../shor.html#L1357)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:1390-1394](../shor.html#L1390-L1394)）は
...
`appDayStr()` （[shor.html:925-928](../shor.html#L925-L928)）が「アプリ内の
...
（[shor.html:929-933](../shor.html#L929-L933)）は同じ区切りでの前日を返す。
...
`renderHome()`の前日結果判定（[shor.html:1169](../shor.html#L1169)）・
`getResultForDate()`の集計期間（[shor.html:1063-1087](../shor.html#L1063-L1087)、
...
- **ホーム画面**（`#btn-see`「浜辺を見に行く」）: 文言・`disabled`の出し分けは
  ...
  （[shor.html:1271-1274](../shor.html#L1271-L1274)。モーダルではなく
