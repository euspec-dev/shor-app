...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:590-591](../shor.html#L590-L591)）
...
（[shor.html:732-739](../shor.html#L732-L739)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:683](../shor.html#L683)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:495-500](../shor.html#L495-L500)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:733](../shor.html#L733)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:766-770](../shor.html#L766-L770)）は
...
`todayStr()` （[shor.html:451-454](../shor.html#L451-L454)）はブラウザの
...
- ホーム画面「誰かの感性に触れる」（[shor.html:1029-1041](../shor.html#L1029-L1041)）
- 投稿完了画面「誰かの感性に触れる」（[shor.html:959-965](../shor.html#L959-L965)）
