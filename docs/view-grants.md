...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:592-593](../shor.html#L592-L593)）
...
（[shor.html:734-741](../shor.html#L734-L741)、`tick()` 内で
...
1. **`peek_drift(viewer_id)`** （候補を選ぶだけ、副作用なし）
   写真を開いた瞬間 `getRandomDrift()`（[shor.html:685](../shor.html#L685)）
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:497-502](../shor.html#L497-L502)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:735](../shor.html#L735)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:768-772](../shor.html#L768-L772)）は
...
`todayStr()` （[shor.html:453-456](../shor.html#L453-L456)）はブラウザの
...
- ホーム画面「誰かの感性に触れる」（[shor.html:1045-1057](../shor.html#L1045-L1057)）
- 投稿完了画面「誰かの感性に触れる」（[shor.html:975-981](../shor.html#L975-L981)）
