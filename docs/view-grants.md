...
- `freeUsedToday()` / `useFreeGrant()` （[shor.html:1132-1133](../shor.html#L1132-L1133)）
...
（[shor.html:1359-1361](../shor.html#L1359-L1361)、`tick()` 内で
...
2. **`confirm_drift(viewer_id, post_id)`** （現像完了時に呼ぶ、ここで確定）
   `confirmDrift()`（[shor.html:1009-1014](../shor.html#L1009-L1014)）から、
   `tick()` 内で `developed` が `true` になった瞬間（[shor.html:1360](../shor.html#L1360)）
   に呼ばれる。ここで初めて:
   - 1日の視聴上限チェック（後述）
...
`release()`（[shor.html:1393-1397](../shor.html#L1393-L1397)）は
...
`appDayStr()` （[shor.html:932-935](../shor.html#L932-L935)）が「アプリ内の
...
（[shor.html:936-940](../shor.html#L936-L940)）は同じ区切りでの前日を返す。
...
`renderHome()`の前日結果判定（[shor.html:1171](../shor.html#L1171)）・
`getResultForDate()`の集計期間（[shor.html:1068-1092](../shor.html#L1068-L1092)、
...
- **ホーム画面**（`#btn-see`「浜辺を見に行く」）: 文言・`disabled`の出し分けは
  ...
  `showViewEmpty()`（[shor.html:1328-1333](../shor.html#L1328-L1333)）が
  「明日の朝　潮が満ちるころに届きます」と案内する
  （[shor.html:1274-1277](../shor.html#L1274-L1277)。モーダルではなく
  画面遷移後の常設表示で、「ホームへ」ボタンで戻る）。以前あった
  「投稿済みかどうかで文言を変える」分岐はこれより前に廃止済み
- **投稿完了画面**「流れ着いた一通を見る」（`#btn-give2see`、
  [shor.html:1871-1877](../shor.html#L1871-L1877)）: こちらは変更なし。
