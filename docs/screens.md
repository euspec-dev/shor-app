...
- 現像インタラクション完了（長押しをやり切って指を離す）後、washのアニメーション
  を経て:
  - `origin==="done"`（[shor.html:788](../shor.html#L788)）→ `renderHome()`
    （→scr-home）。投稿完了画面経由で見た場合も、現像後は投稿完了画面には
    戻らずホームへ抜ける（意図的な仕様。「候補0件」で見られなかった場合の
    `backToDone()`分岐（[shor.html:690](../shor.html#L690)）とは扱いが違う点に注意）
  - それ以外 → `openPost()`（[shor.html:789](../shor.html#L789)、→scr-post）
...
- 表示関数: `openDone()`（[shor.html:949](../shor.html#L949)、投稿直後）
  / `backToDone()`（[shor.html:950](../shor.html#L950)、閲覧起点がdoneで
