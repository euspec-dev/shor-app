...
（[shor.html:733](../shor.html#L733)）が`.active`クラスを付け替えることで
...
- 表示関数: `renderHome()`（[shor.html:770](../shor.html#L770)）
...
- 表示関数: `openView(origin)`（[shor.html:817](../shor.html#L817)）。
...
- `peek_drift`で候補が0件だった場合、または候補は返ってきたが
  ...
  [shor.html:808-815](../shor.html#L808-L815) / [shor.html:839](../shor.html#L839)）:
  どちらも同じ扱いで、まず`origin==="done"`なら`backToDone()`、それ以外は
  `renderHome()`で先に画面を戻してから、`notice-modal`
  （[shor.html:845-846](../shor.html#L845-L846)）で「まだ写真が届いていません。
...
- 候補の取得に成功すると、写真がすぐには見えず前置き演出を挟む
  （[shor.html:854-862](../shor.html#L854-L862)）:
...
- 現像インタラクション完了（長押しをやり切って指を離す）後、washのアニメーション
  を経て:
  - `origin==="done"`（[shor.html:937](../shor.html#L937)）→ `renderHome()`
    （→scr-home）。投稿完了画面経由で見た場合も、現像後は投稿完了画面には
    戻らずホームへ抜ける（意図的な仕様。「候補0件」で見られなかった場合の
    `backToDone()`分岐（[shor.html:845](../shor.html#L845)）とは扱いが違う点に注意）
  - それ以外 → `openPost()`（[shor.html:938](../shor.html#L938)、→scr-post）
- 長押しを最後までやり切らずに離した場合はこの画面に留まり、同じ写真に
  再挑戦できる（`resetDevelop(false)`、[develop-interaction.md](develop-interaction.md)参照）

## scr-post（画面3: 投稿）

- 表示関数: `openPost()`（[shor.html:973](../shor.html#L973)）
...
- 表示関数: `openDone()`（[shor.html:1140](../shor.html#L1140)、投稿直後）
  / `backToDone()`（[shor.html:1141](../shor.html#L1141)、閲覧起点がdoneで
...
- ボタン:
  ...
  - `btn-pwa`「ホーム画面に追加する」→ PWAインストールプロンプト
    ...
    [shor.html:1175-1196](../shor.html#L1175-L1196)）

## モーダル（画面遷移ではなく重ね表示）

`.screen`とは別に、現在の画面の上に重ねて出すモーダルが3つある
（`showModal()`/`hideModal()`, [shor.html:1201-1207](../shor.html#L1201-L1207)）。
