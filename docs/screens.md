...
- 表示関数: `openDone()`（[shor.html:1165](../shor.html#L1165)、投稿直後）
  / `backToDone()`（[shor.html:1166](../shor.html#L1166)、閲覧起点がdoneで
...
- ボタン:
  ...
  - `btn-pwa`「ホーム画面に追加する」→ PWAインストールプロンプト
    ...
    [shor.html:1200-1223](../shor.html#L1200-L1223)）

## モーダル（画面遷移ではなく重ね表示）

`.screen`とは別に、現在の画面の上に重ねて出すモーダルが3つある
（`showModal()`/`hideModal()`, [shor.html:1228-1234](../shor.html#L1228-L1234)）。
