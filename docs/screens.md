...
（[shor.html:1047](../shor.html#L1047)）が`.active`クラスを付け替えることで
...
- 表示関数: `renderHome()`（[shor.html:1137-1206](../shor.html#L1137-L1206)）
...
DOM上の並び順は `#home-a` → `#home-to-post`（主ボタン、常時） →
...
- 巻紙が開く演出（`.unroll-stage`, [shor.html:249-361](../shor.html#L249-L361)）は、
  ...
  - **clipの境界は「筒の中心」に合わせてある**（筒の上端ではない）。筒は
    ...
    値で動く（[shor.html:340-343](../shor.html#L340-L343)）。以前はこの補正が
  ...
  - `rollGo`のopacityは以前は`top`/`height`と同じキーフレーム内（86%地点）
    ...
    完全に同期させている
  - 演出中（`.rolling`クラスが付いている間、[shor.html:277](../shor.html#L277)、
  ...
  - `prefers-reduced-motion: reduce`環境では、演出させずに開き切った状態へ
    即座に切り替える（[shor.html:357-361](../shor.html#L357-L361)）
  - フェードイン/アウトの所要時間そのもの（前置きメッセージ・ホーム→scr-viewの
    ...
    さらわれる）アニメーションにのみ使われている（[shor.html:235](../shor.html#L235)）
...
- `myPosts`が空（一度も投稿していない）のときだけ、`#post-photo`に
  ...
  `DUMMY_CAPTION`定数、[shor.html:935-938](../shor.html#L935-L938)。
...
**DOM構成**（`#post-photo`, [shor.html:779-790](../shor.html#L779-L790)）:
...
[shor.html:563-569](../shor.html#L563-L569)）、`#post-dummy`（見本表示、
上記参照）を重ねている。`#post-photo`自体は`touch-action:none`
（[shor.html:556](../shor.html#L556)）でブラウザ標準のスクロール/ズームを
...
受け取り演出（scr-viewの`.unroll-stage`、[shor.html:249-361](../shor.html#L249-L361)）
...
（[shor.html:363-479](../shor.html#L363-L479)）。
...
1. **ポラロイドが上へ巻かれる**（0〜2.2s）: 巻かれるのはアニメ専用の複製
   ...
   [shor.html:381-389](../shor.html#L381-L389)）の`clip-path`が
   `inset(0 0 4.5px 0)`→`inset(0 0 calc(100% - 13px) 0)`へ変化する
   （`sendoffRollUp`, [shor.html:430-433](../shor.html#L430-L433)）。
   ...
   `sendoffClipFinish`（[shor.html:436-439](../shor.html#L436-L439)）を
   ...
   自身の`box-shadow`（`.polaroid`, [shor.html:221-222](../shor.html#L221-L222)）も
   この`overflow:hidden`で同じ境界に収まる。

   `.sendoff-shadow`には見た目に影響しない`transform:translateZ(0)`
   （[shor.html:373-380](../shor.html#L373-L380)）も付けてある。受け取り
   ...
   （[shor.html:399-402](../shor.html#L399-L402)）で演出中は
...
3. **ボトル画像がフェードイン**（3.2〜4.0s。`rotate(-3deg)`固定、
   ...
   [shor.html:803-805](../shor.html#L803-L805)）。`.sendoff-bottle`自体の
...
として表示している（`<p class="passage">`、[shor.html:814](../shor.html#L814)）。
...
- ボタン:
  ...
  - `btn-pwa`「ホーム画面に追加する」→ PWAインストールプロンプト
    ...
    （[shor.html:194-205](../shor.html#L194-L205)、JS側は
...
- フォアグラウンド復帰のたび（`visibilitychange`が`visible`、または
  ...
  （[shor.html:636-643](../shor.html#L636-L643)）を出す
