...
[dev-workflow.md](dev-workflow.md)（このチェックリストを使うタイミング）。

このリストは毎リリースのログではなく、常に使い回す確認項目のテンプレート。
- **項目を追加するタイミング**: コード/インフラの変更で「リリース前に確認しないと
  事故る新しいリスク」が生まれたとき（新しい環境変数、新しいマイグレーション、
  新しい外部依存の追加など）
- **項目を削除するタイミング**: そのリスクが恒久的に消えたとき（該当機能自体を
  削除した、もしくはCIやコードで自動検証されるようになって人力チェックが
  不要になった）
...
- [ ] 投稿写真の中身検証スキップ（`DEV_SKIP_PHOTO_CHECK`）が本番で無効
