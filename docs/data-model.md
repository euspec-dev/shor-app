...
- `handlePickedFile(file, fromCamera)`（[shor.html:1020-1036](../shor.html#L1020-L1036)）
  - `fromCamera=true`（`camera-input`、`capture="environment"`経由）:
    撮ったばかりの写真は定義上カメラ写真なので、EXIF/HEIC判定を
    丸ごとスキップして無条件で受け付ける
  - `fromCamera=false`（`file-input`＝ギャラリー経由）:
    従来通り`hasCameraExif()`（[shor.html:1102](../shor.html#L1102)、
    JPEGのEXIF Make/Modelタグの有無を見る簡易パーサ）または`isHeic()`
    （[shor.html:1136](../shor.html#L1136)、HEIC/HEIFのシグネチャ判定。
...
- 経路を確定させるため、`btn-pick`「＋ 写真を選ぶ」を押すと、iOS/Androidを
  ...
  明示的に振り分けている（[shor.html:1002](../shor.html#L1002)）
