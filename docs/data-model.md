...
（[shor.html:977-984](../shor.html#L977-L984)）。
...
- 書き込み: `btnSend`のクリックハンドラ内、`createPost()`成功直後
  （[shor.html:1755-1759](../shor.html#L1755-L1759)）。サムネイルは
  `makeThumb(finalDataUrl, 240, .5)`（[shor.html:1854-1866](../shor.html#L1854-L1866)）
  ...
  経由しない
- 掃除: `pruneMyPostThumbs()`（[shor.html:1040-1044](../shor.html#L1040-L1044)）が
...
- 読み出し: `renderHome()`が起動の度に`shor:myPosts`を読み、まだ結果を
  ...
  （[shor.html:1110-1116](../shor.html#L1110-L1116)）で`view_history`の
...
| カラム | 型 | 備考 |
|---|---|---|
| `id` | uuid (PK) | クライアントが`genUUID()`（[shor.html:977-984](../shor.html#L977-L984)）で生成し、以後永続化する匿名ID |
...
- `THEMES`（[shor.html:956-965](../shor.html#L956-L965)）: 30個の固定文言の
...
- `themeFor(dayStr = appDayStr())`（[shor.html:968-971](../shor.html#L968-L971)）:
...
- 開発バーの「テーマ閲覧」（`IS_DEV`限定、[shor.html:2010-2018](../shor.html#L2010-L2018)）: `peek_drift`を呼ばず、`theme: themeFor()`
...
（[shor.html:1049](../shor.html#L1049)）。`genUUID()`は`crypto.randomUUID()`が
...
組み立てるフォールバックに切り替える（[shor.html:977-984](../shor.html#L977-L984)）。
...
- `isPng(buffer)`（[shor.html:1828-1834](../shor.html#L1828-L1834)）
  PNGのシグネチャ（先頭8バイト）を見るだけの単純な判定。EXIF解析はしない。
- `handlePickedFile(file, fromCamera)`（[shor.html:1719-1734](../shor.html#L1719-L1734)）
...
- **iOS**: `btn-pick`「＋ 写真を選ぶ」を押すと自前モーダルを挟まず
  `file-input`を直接開く（[shor.html:1697-1699](../shor.html#L1697-L1699)、
