...
（[shor.html:600-601](../shor.html#L600-L601)）。
...
（[shor.html:657](../shor.html#L657)）。`posts.image_url`にはパスではなく
...
- `handlePickedFile(file, fromCamera)`（[shor.html:1008-1024](../shor.html#L1008-L1024)）
  - `fromCamera=true`（`camera-input`、`capture="environment"`経由）:
    撮ったばかりの写真は定義上カメラ写真なので、EXIF/HEIC判定を
    丸ごとスキップして無条件で受け付ける
  - `fromCamera=false`（`file-input`＝ギャラリー経由）:
    従来通り`hasCameraExif()`（[shor.html:1090](../shor.html#L1090)、
    JPEGのEXIF Make/Modelタグの有無を見る簡易パーサ）または`isHeic()`
    （[shor.html:1124](../shor.html#L1124)、HEIC/HEIFのシグネチャ判定。
...
- 経路を確定させるため、`btn-pick`「＋ 写真を選ぶ」を押すと、iOS/Androidを
  ...
  明示的に振り分けている（[shor.html:990](../shor.html#L990)）
