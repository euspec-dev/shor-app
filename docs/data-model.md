...
（[shor.html:946-959](../shor.html#L946-L959)）。
...
- 書き込み: `btnSend`のクリックハンドラ内、`createPost()`成功直後
  （[shor.html:1728-1730](../shor.html#L1728-L1730)）。サムネイルは
  `makeThumb(finalDataUrl, 240, .5)`（[shor.html:1825-1837](../shor.html#L1825-L1837)）
  ...
  経由しない
- 掃除: `pruneMyPostThumbs()`（[shor.html:1013-1017](../shor.html#L1013-L1017)）が
...
- 読み出し: `renderHome()`が起動の度に`shor:myPosts`を読み、まだ結果を
  ...
  （[shor.html:1083-1089](../shor.html#L1083-L1089)）で`view_history`の
...
| カラム | 型 | 備考 |
|---|---|---|
| `id` | uuid (PK) | クライアントが`genUUID()`（[shor.html:950-957](../shor.html#L950-L957)）で生成し、以後永続化する匿名ID |
...
- `THEMES`（[shor.html:924-933](../shor.html#L924-L933)）: 30個の固定文言の
...
- `themeFor(dayStr = appDayStr())`（[shor.html:936-939](../shor.html#L936-L939)）:
...
- 開発バーの「テーマ閲覧」（`IS_DEV`限定、[shor.html:2013-2021]
  (../shor.html#L2013-L2021)）: `peek_drift`を呼ばず、`theme: themeFor()`
...
（[shor.html:1022](../shor.html#L1022)）。`genUUID()`は`crypto.randomUUID()`が
...
組み立てるフォールバックに切り替える（[shor.html:950-957](../shor.html#L950-L957)）。
...
- `isPng(buffer)`（[shor.html:1830-1836](../shor.html#L1830-L1836)）
  PNGのシグネチャ（先頭8バイト）を見るだけの単純な判定。EXIF解析はしない。
- `handlePickedFile(file, fromCamera)`（[shor.html:1689-1705](../shor.html#L1689-L1705)）
...
- **iOS**: `btn-pick`「＋ 写真を選ぶ」を押すと自前モーダルを挟まず
  `file-input`を直接開く（[shor.html:1642-1656](../shor.html#L1642-L1656)、
