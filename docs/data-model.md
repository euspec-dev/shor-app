...
（[shor.html:769-782](../shor.html#L769-L782)）。
...
- 書き込み: `btnSend`のクリックハンドラ内、`createPost()`成功直後
  （[shor.html:1289-1291](../shor.html#L1289-L1291)）。サムネイルは
  `makeThumb(pickedDataUrl, 240, .5)`（[shor.html:1394-1406](../shor.html#L1394-L1406)）
  で生成する。`downscale()`（写真投稿本体のアップロード用）とは別の
  軽量版で、既にJPEG化済みのdataURLを再度縮小するだけなのでFile/Blobを
  経由しない
- 掃除: `pruneMyPostThumbs()`（[shor.html:836-840](../shor.html#L836-L840)）が
...
- 読み出し: `renderHome()`が起動の度に`shor:myPosts`を読み、まだ結果を
  ...
  （[shor.html:898-904](../shor.html#L898-L904)）で`view_history`の
...
| カラム | 型 | 備考 |
|---|---|---|
| `id` | uuid (PK) | クライアントが`genUUID()`（[shor.html:773-780](../shor.html#L773-L780)）で生成し、以後永続化する匿名ID |
...
（[shor.html:845](../shor.html#L845)）。`genUUID()`は`crypto.randomUUID()`が
...
組み立てるフォールバックに切り替える（[shor.html:773-780](../shor.html#L773-L780)）。
...
- `isPng(buffer)`（[shor.html:1368-1374](../shor.html#L1368-L1374)）
  PNGのシグネチャ（先頭8バイト）を見るだけの単純な判定。EXIF解析はしない。
- `handlePickedFile(file, fromCamera)`（[shor.html:1251-1267](../shor.html#L1251-L1267)）
...
- **iOS**: `btn-pick`「＋ 写真を選ぶ」を押すと自前モーダルを挟まず
  `file-input`を直接開く（[shor.html:1218-1232](../shor.html#L1218-L1232)、
